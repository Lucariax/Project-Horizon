...
public final class GameGraphics implements ImageObserver {
	// class constants
	...
	private static final String ABOUT_MESSAGE = "   Programmers:  \n"
			...
			+ " Emery J. Martz \n";
	private static final String ABOUT_MESSAGE_TITLE = "About The Training Room";
	private static final String COURSE_WEB_SITE = ".";
	private static final String TITLE = "The Training Room";
	public static final String ANIMATED_PROPERTY = "gamegraphics.animated";
	public static final String ANIMATION_FILE_NAME = "_gamegraphics_animation_save.txt";
	public static final String AUTO_ENABLE_ANIMATION_ON_SLEEP_PROPERTY = "gamegraphics.animateonsleep";
	public static final String DIFF_PROPERTY = "gamegraphics.diff";
	public static final String HEADLESS_PROPERTY = "gamegraphics.headless";
	public static final String MULTIPLE_PROPERTY = "gamegraphics.multiple";
	public static final String SAVE_PROPERTY = "gamegraphics.save";
	...
	// last GameGraphics is closed
	static {
		try {
			String debugProp = String.valueOf(System.getProperty("gamegraphics.debug")).toLowerCase();
			DEBUG = DEBUG || "true".equalsIgnoreCase(debugProp) || "on".equalsIgnoreCase(debugProp)
					|| "yes".equalsIgnoreCase(debugProp) || "1".equals(debugProp);
		} catch (Throwable t) {
			// empty
		}
	}
	...
	/*
	 * Called when GameGraphics class loads up. Checks whether the user wants to
	...
	private static void checkAnimationSettings() {
		try {
			...
			if (settingsFile.exists()) {
				...
				System.out.println("***");
				System.out
						.println("*** GameGraphics saving animated GIF: " + new File(animationSaveFileName).getName());
				System.out.println("***");
				settingsFile.delete();
	...
	/**
	 * Returns the red component of the given RGB pixel from 0-255. Often used in
	...
	/**
	 * Returns the green component of the given RGB pixel from 0-255. Often used in
	 * conjunction with the methods getPixelRGB, setPixelRGB, etc.
	 */
	...
	/**
	 * Returns the blue component of the given RGB pixel from 0-255. Often used in
	 * conjunction with the methods getPixelRGB, setPixelRGB, etc.
	 */
	...
	/*
	 * Returns whether the 'main' thread is still running. Used to determine whether
	 * to exit the program when the drawing panel is closed by the user.
	 */
	public static boolean mainIsActive() {
		...
		for (int i = 0; i < threads.length; i++) {
			...
			if (DEBUG)
				System.out.println("    GameGraphics.mainIsActive(): " + thread.getName() + ", priority="
						+ thread.getPriority() + ", alive=" + thread.isAlive() + ", stack="
	 ...
	 * from 0-255. The returned integer is suitable for use with various RGB integer
	 * methods in this class such as setPixel.
	 */
	...
	/*
	 * Returns whether the current program is running in the DrJava editor. This was
	 * needed in the past because DrJava messed with some settings.
	 */
	...
	public GameGraphics(int width, int height) {
		...
		if (DEBUG)
			System.out.println("GameGraphics(): going to grab lock");
		synchronized (LOCK) {
			instances++;
			...
			if (shutdownThread == null && !usingDrJava()) {
				if (DEBUG)
					System.out.println("GameGraphics(): starting idle thread");
				shutdownThread = new Thread(new Runnable() {
					...
					public void run() {
						...
						try {
							while (true) {
								// maybe shut down the program, if no more GameGraphicss are onscreen
								// and main has finished executing
								save |= shouldSave();
								if (DEBUG)
									System.out.println("GameGraphics idle thread: instances=" + instances + ", save="
											+ save + ", main active=" + mainIsActive());
								if ((instances == 0 || save) && !mainIsActive()) {
									try {
										...
										if (DEBUG)
											System.out.println(
													"GameGraphics idle thread: unable to exit program: " + sex);
									}
								}
							...
							if (DEBUG)
								System.out.println("GameGraphics idle thread: exception caught: " + e);
						}
					}
				});
				// shutdownThread.setPriority(Thread.MIN_PRIORITY);
				shutdownThread.setName("GameGraphics-shutdown");
		...
		if (DEBUG)
			System.out.println("GameGraphics(w=" + width + ",h=" + height + ",anim=" + isAnimated() + ",graph="
			...
			*/
			...
			try {
				Runtime.getRuntime().addShutdownHook(new Thread(new Runnable() {
					...
					public void run() {
						if (DEBUG)
							System.out.println("GameGraphics.run(): Running shutdown hook");
						if (DEBUG)
							System.out.println("GameGraphics shutdown hook: instances=" + instances);
						try {
				...
				if (DEBUG)
					System.out.println("GameGraphics(): unable to add shutdown hook: " + e);
			}
		}
	}
...
//    }
	...
	/*
	 * Whether the panel should automatically switch to animated mode if it calls
	 * the sleep method.
	 */
	...
	/*
	 * Compares the current GameGraphics image to an image file on disk.
	 */
	...
	/*
	 * Compares the current GameGraphics image to an image file on the web.
	 */
	...
	/*
	 * Closes the GameGraphics and exits the program.
	 */
	private void exit() {
		if (isGraphical()) {
	 ...
	 * occurrences calls on various drawing methods. Experts can manipulate the map,
	 * but novices will be encouraged to simply print it.
	 */
	...
	/**
	 * A variation of getGraphics that returns an object that records a count for
	...
	private BufferedImage getImage() {
		...
		// if (DEBUG) System.out.println("GameGraphics getImage setting background to "
		// + backgroundColor);
		g.setColor(backgroundColor);
	 ...
	 * extracted from this by calling GameGraphics.getRed, getGreen, and getBlue. If
	 * nothing has been explicitly drawn on this particular pixel, the panel's
	 * background color is returned. See also: getPixel.
	 */
	...
	/**
	 * Returns the colors of all pixels in this GameGraphics as a 2-D array of Color
	 * objects. The first index of the array is the y-coordinate, and the second
	 * index is the x-coordinate. So, for example, index [r][c] represents the RGB
	...
	/**
	 * Returns the colors of all pixels in this GameGraphics as a 2-D array of RGB
	 * integers. The first index of the array is the y-coordinate, and the second
	 * index is the x-coordinate. So, for example, index [r][c] represents the RGB
	 * pixel data for the pixel at position (x=c, y=r). The individual red, green,
	 * and blue components of each RGB integer can be extracted from this by calling
	 * GameGraphics.getRed, getGreen, and getBlue.
	 */
	...
	/**
	 * Required method of ImageObserver interface. Notifies the panel when images
	 * are loaded and updated.
	 */
	@Override
	...
	/*
	 * Returns whether this drawing panel is going to be displayed on screen. This
	 * is almost always true except in some server environments where the
	 * GameGraphics is run 'headless' without a GUI, often for scripting and
	 * automation purposes.
	 */
	...
	/*
	 * Returns true if the drawing panel class is in multiple mode. This would be
	 * true if the current program pops up several drawing panels and we want to
	 * save the state of each of them to a different file.
	 */
	...
	public Image loadImage(String filename) {
		if (!(new File(filename)).exists()) {
			throw new RuntimeException("GameGraphics.loadImage: File not found: " + filename);
		}
	...
	/*
	 * Returns whether the drawing panel should be closed and the program should be
	 * shut down.
	 */
	...
	public void saveAnimated(String filename) throws IOException {
		...
		if (DEBUG)
			System.out.println("GameGraphics.saveAnimated(" + filename + ")");
	...
	/*
	 * Called when the user presses the "Save As" menu item. Pops up a file chooser
	 * prompting the user to save their panel to an image.
	 */
	private void saveAs() {
	...
	private void saveAsAnimated() {
		...
		if (filename != null) {
			try {
				...
				JOptionPane.showMessageDialog(frame,
						"Due to constraints about how GameGraphics works, you'll need to\n"
								+ "re-run your program.  When you run it the next time, GameGraphics will \n"
	...
	/*
	 * Saves the drawing panel's image to a temporary file and returns that file's
	 * name.
	 */
	...
	/**
	 * Sets the background color of the drawing panel to be the color represented by
	...
	/**
	 * Sets the drawing panel's height in pixels to the given value. After calling
	 * this method, the client must call getGraphics() again to get the new graphics
	 * context of the newly enlarged image buffer.
	 */
	...
	/**
	 * Sets the colors of all pixels in this GameGraphics to the colors in the given
	 * 2-D array of Color objects. The first index of the array is the y-coordinate,
	 * and the second index is the x-coordinate. So, for example, index [r][c]
	 * represents the RGB pixel data for the pixel at position (x=c, y=r). If the
	 * given array's dimensions do not match the width/height of the drawing panel,
	 * the panel is resized to match the array. If the pixel array is null or size
	 * 0, the call has no effect. If any rows or colors in the array are null, those
	 * pixels will be ignored. The 2-D array passed is assumed to be rectangular in
	 * length (not jagged).
	 */
	...
	/**
	 * Sets the colors of all pixels in this GameGraphics to the colors represented
	 * by the given 2-D array of RGB integers. The first index of the array is the
	 * y-coordinate, and the second index is the x-coordinate. So, for example,
	 * index [r][c] represents the RGB pixel data for the pixel at position (x=c,
	 * y=r). If the given array's dimensions do not match the width/height of the
	 * drawing panel, the panel is resized to match the array. If the pixel array is
	 * null or size 0, the call has no effect. The 2-D array passed is assumed to be
	 * rectangular in length (not jagged).
	 */
	 ...
	 * After calling this method, the client must call getGraphics() again to get
	 * the new graphics context of the newly enlarged image buffer.
	 */
	...
	/**
	 * Sets the drawing panel's width in pixels to the given value. After calling
	 * this method, the client must call getGraphics() again to get the new graphics
	 * context of the newly enlarged image buffer.
	 */
	...
	/*
	 * Returns whether the user wants to save the drawing panel contents to a file
	 * automatically.
	 */
	...
	/**
	 * Zooms the drawing panel in/out to the given factor. A zoom factor of 1, the
	 * default, indicates normal size. The factor value passed should be >= 1; if
	 * not, 1 will be used.
	 */
	...
	private class DPWindowListener extends WindowAdapter {
		// called when GameGraphics closes, to potentially exit the program
	...
	/*
	 * This inner class passes through calls to the panel's Graphics object g2 but
	 * also records a count of how many times various basic drawing methods are
	 * called. This is used for debugging purposes, so that a client can compare
	 * their counts of various graphical method calls to those from an expected
	 * output as a "sanity check" on their program's behavior. Notice that it
	 * extends Graphics and not Graphics2D, so it is more limited than g2.
	 * 
	 * @author Stuart Reges
	 */
	 ...
	 * screen to report the differences between two images. It is used to allow the
	 * client to compare their program's output against a known correct output and
	 * view which pixels differ between the two.
	 */
